...
pub trait KeyPartsSequence {
    ...
    fn fmt_debug(
        ...
        format_struct(parts, suffix, None, f)
    }
}
...
impl<T: KeyPartsSequence> Key<T> {
    ...
    pub fn get_suffix(&self) -> Option<&[u8]> {
        let suffix = &self.bytes
            [self.bytes.len() - self.suffix_len - self.key_len..self.bytes.len() - self.suffix_len];
...
impl<T: KeyPartsSequence> std::fmt::Debug for Key<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let suffix_bytes = {
            let bytes = &self.bytes[self.bytes.len() - self.suffix_len - self.key_len
                ..self.bytes.len() - self.key_len];
            match bytes.len() {
                0 => None,
                _ => Some(bytes),
            }
...
mod tests {
    ...
    fn key_seq_with_suffix_test() {
        ...
        assert_eq!(key_seq.suffix, Some(vec![50, 60]),);
    ...
    fn key_get_key_test() {
        ...
        assert_eq!(key.get_key(), Some(expected),)
    }
    ...
    fn key_get_suffix_test() {
        ...
        assert_eq!(key.get_suffix(), Some(expected))
    }
